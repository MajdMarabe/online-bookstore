...
LEFT JOIN language C ON C.language_id = B.language_id 
";
$all_product= $mysqli->query($sql);

$o="SELECT * FROM author  ";
$auth= $mysqli->query($o);
...
<section>
    ...
    <div style="display: flex ; flex-direction: column ">
        <!-- 1--> <div style="box-shadow:  2px 2px 10px #000; background-color:#f6fdf6; width:  100%; height: 30%; padding-top:5px;text-align: center;font-size: 60px; margin-top:140px;border-bottom: 1px solid #d1e9c8">  <h1 style="margin-top: 70px">Books Info</h1> </div>
...
<form class="a" method="get" action="addbook.php">
    <span>isbn</span>
    <input name="isbn" type="number"min="1">
    <span>Book name</span>
    <input name="bookname"type="text">
    <span>picture for the book</span>
    <input name="picture" type="text" placeholder="pic.png">
    ...
    <span>Quantity</span>
    <input name="allqantity"type="number"min="1">
                <span>Price</span>
    ...
    <?php while($author=mysqli_fetch_assoc($auth)){?>
        <option value="<?php echo $author['author_id']?>"><?php echo $author['authorname']?></option>

      <?php   } ?>
    </select>


<button class="delete">add</button>
...
<table style="">
    ...
    <td>quantity</td>
    <td>author name</td>
        <td>New Arrivals & Global Trends
            <?php
            if(isset($_SESSION["five"])){
                ?>
            <pre style="font-family:cursive ; font-size: 17px; color: red"> you can add only five books </pre>
<?php
                unset($_SESSION["five"]);
            }
            ?>
    ...
    while($row=mysqli_fetch_assoc($all_product)){?>
        ...
        <td><?php echo  $row['languagename'];?></td>
            <td><?php echo  $row['allqantity'];?></td>
            <td><?php echo  $row['authorname'];?></td>

            <td>  <a href="addtrend.php?isbn=<?php echo $row['isbn']; ?>&name=<?php echo $row['bookname']; ?>&pic=<?php echo $row['picture']; ?>&price=<?php echo  $row['price'];?>&lang=<?php echo  $row['languagename'];?>&quan=<?php echo  $row['allqantity'];?>&auth=<?php echo  $row['authorname'];?>" > <button class="delete">  add</button> </a>  </td>
            <td>  <a href="deletebook.php?isbn=<?php echo $row['isbn']; ?>" >

                    <button style="margin-left: 30px;margin-right: 30px" class="delete">  delete</button> </a>  </td>
    </tr>
        <?php
    }
    ?>
